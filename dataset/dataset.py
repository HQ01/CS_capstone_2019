...
class TUDataset(Dataset):
    ...
    def __init__(self, name, hidden_size=10):
        ...
        try:
            ...
            if len(DS_node_attr.shape) == 1:
                DS_node_attr = DS_node_attr.reshape(-1, 1)
            for idxs, g in zip(node_idx_list, self.graph_lists):
        ...
        if 'feat' not in g.ndata.keys():
            ...
            print("Use Constant as Feature")

    def __getitem__(self, idx):
        g = self.graph_lists[idx]
        return (g.adjacency_matrix().to_dense(), g.ndata['feat']), self.graph_labels[idx], None
...
class CollateFn:
    def __init__(self, max_num_nodes, device='cpu'):
